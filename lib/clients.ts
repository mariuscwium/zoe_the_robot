/** Production implementations of dependency interfaces. */

import { Redis } from "@upstash/redis";
import Anthropic from "@anthropic-ai/sdk";
import type {
  RedisClient, RedisResult, TelegramClient, TelegramResult, TelegramMessage,
  TelegramFile, SetWebhookParams, CalendarClient, CalendarEvent, CalendarEventInput,
  CalendarEventList, ListEventsParams, ClaudeClient, ClaudeMessage, ClaudeMessageParams,
  TranscriptionClient, Clock,
} from "./deps.js";
...
export function createRedisClient(url: string, token: string): RedisClient {
  ...
  return {
    ...
    async pipeline(commands: string[][]): Promise<RedisResult[]> {
      ...
      return (await p.exec()).map((r) => ({ result: r }));
    },
  };
}
...
interface GoogleTokenCache { accessToken: string; expiresAt: number }
...
async function calFetch<T>(
  calBase: string, path: string, tokenGetter: () => Promise<string>, init?: RequestInit,
): Promise<T> {
  ...
  return {
    async createMessage(params: ClaudeMessageParams): Promise<ClaudeMessage> {
      const response = await client.messages.create({
        model: params.model, max_tokens: params.max_tokens, system: params.system,
...
export function createTranscriptionClient(apiKey: string): TranscriptionClient {
  return {
    async transcribe(buffer: Buffer, mimeType: string): Promise<string> {
      ...
      if (!res.ok) throw new Error(`Whisper API error ${String(res.status)}: ${await res.text()}`);
